...
const Signup = () => {
  ...
  const HandleSubmit = async (e) => {
    ...
    // Basic validation
    let hasError = false;
    const newErrors = { name: '', email: '', password: '', cpassword: '', type:'', };
    ...
    const { name, email, password, cpassword, type } = state;
    const res = await fetch("http://localhost:5000/signup", {
      ...
      body: JSON.stringify({
        name, email, password, cpassword, type,
      })
    ...
    if (datahit.status === 422 || !datahit) {
      ...
      localStorage.setItem('token', datahit.token);
      history("/login")
      setState({ ...state, name: "", email: "", password: "", cpassword: "", type:"", })
    }
      ...
      <div className='handle_form d-flex flex-column p-3'>
        ...
        <form method='POST' onSubmit={HandleSubmit}>
          <div className='col-12 col-lg-10 m-auto'>
            ...
            <select className="form-select" name="type" value={state.type} onChange={HandleSelect}>
              <option value="">Select type</option>
              <option value="Admin">Admin</option>
              <option value="Broker">Broker</option>
            </select>
            <span className="error" style={{ color: "red" }}>{errors.type}</span>
          </div>
